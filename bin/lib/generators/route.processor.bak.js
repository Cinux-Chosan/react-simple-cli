...
class RouteProcesser {
    ...
    checkAndGetRoutes(node) {
        ...
        if (routesPropNode && t.isArrayExpression(routesPropNode.value)) {
            // 
        } else {
    ...
    findMatchRoute(nodeList, path) {
        return nodeList.find(node => {
            const typePropNode = this._getObjNodeProperty(node, 'type')
            switch (typePropNode.value.value) {
                case 'switch':
                    const routesPropNode = this.checkAndGetRoutes(node, 'routes')
                    return routesPropNode.value.elements.find(route => {
                        const routesProp = this._getObjNodeProperty(route, 'routes')
                        if (routesProp) {
                            return this.findMatchRoute(routesProp.value.elements, path)
                        }
                    })
                case 'redirect':
                    break
    ...
    getSwitchNode(node) {
        const routeProp = this.checkAndGetRoutes(node)
        return routeProp.value.elements.find(route => {
            const routeTypePropNode = this._getObjNodeProperty(route, 'type')
            return routeTypePropNode.value.value === 'switch'
        })
    }
    ...
    _add(routeParts, rootNode, componentPath) {
        ...
        let parentNode = this.findMatchRoute(switchRoutesNode.value.elements, parentName)
        if (!parentNode) {
            ...
            switch (rootNodeType) {
                ...
                default: {
                    let switchNode = this.getSwitchNode(rootNode)
                    if (!switchNode) {
